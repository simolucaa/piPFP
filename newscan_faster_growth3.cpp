...
static void save_update_word(Args &arg, uint64_t currenWordLength,
                             uint64_t hash, unordered_set<uint64_t> &freq);
...
// #ifndef NOTHREADS
#include "newscan_faster_growth3.hpp"
// #endif
...
static void save_update_word(Args &arg, uint64_t currentSequence, uint64_t hash,
                             unordered_set<uint64_t> &freq) {
  ...
  void operator()(const blocked_range<uint32_t> &r) const {
    ...
    for (uint32_t i = r.begin(); i != r.end(); i++) {
      for (auto &y : wordFreq[i]) {
        auto toInsert = pair<uint64_t, uint32_t>({y.first, y.second.second});
        auto inserted = cumulativeWordFreq.insert(
            a, toInsert); // {y.first, y.second.second});
        if (inserted == false) {
          a->second += y.second.second;
        }
        a.release();
      }
      // delete hash map
...
int main(int argc, char **argv) {
   ...
   */
  vector<unordered_set<uint64_t>> wordFreq(arg.th);
  concurrent_hash_map<uint64_t, uint32_t> cumulativeWordFreq;
