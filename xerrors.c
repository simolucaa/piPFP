...
void xperror(int en, char *msg) {
  ...
  int errmsg = strerror_r(en, buf, Buflen);
  if (msg != NULL)
    fprintf(stderr, "%s: %s\n", msg, errmsg);
  else
    fprintf(stderr, "%s\n", errmsg);
...
int xpthread_join(pthread_t thread, void **retval, int linea,
                  const char *file) {
...
int xpthread_mutex_destroy(pthread_mutex_t *mutex, int linea,
                           const char *file) {
