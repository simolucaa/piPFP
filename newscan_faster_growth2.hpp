...
typedef struct {
  vector<unordered_map<uint64_t, pair<uint32_t, uint32_t>>>
...
void *mt_parse(void *dx) {
  ...
  Args *arg = d->arg;
  unordered_map<uint64_t, pair<uint32_t, uint32_t>> *wordFreq =
      &d->wordFreq->data()[d->nth_thread];
  ...
  save_update_word(*arg, currentWordLength, currentHash, *wordFreq);
...
uint32_t
mt_process_file(Args &arg,
                vector<unordered_map<uint64_t, pair<uint32_t, uint32_t>>> &wf) {
  // get input file size
  ...
  std::string().swap(line); // free memory
  ...
  if (arg.th > line_pos.size()) {
    arg.th = line_pos.size() - 1;
  }
