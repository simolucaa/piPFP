...
static void save_update_word(Args& arg, string& w, concurrent_unordered_map<uint64_t,uint64_t>&  freq, FILE *tmp_parse_file, FILE *last, FILE *sa, uint64_t &pos);
...
// #endif  
  ...
  // auto pair = freq.insert({hash,word_stats()});
  // bool pair_second = pair.second;
  ...
  //       exit(1);
  //     }
  //     // if (freq[hash].len != w.size()) {
  //     //   cerr << "Emergency exit! Hash collision for strings:\n";
  //     //   cerr << "Hash: " << hash << "  Occ: " << freq[hash].occ << endl;
  //     //   cerr << freq[hash].len << "\n  vs\n" <<  w.size() << endl;
  //     //   exit(1);
  //     // }
  //     // if(freq[hash].str != w) {
