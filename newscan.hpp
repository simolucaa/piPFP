...
typedef struct {
  unordered_map<uint64_t,word_stats> *wordFreq; // shared dictionary
...
void *mt_parse(void *dx)
{
  ...
  Args *arg = d->arg;
  unordered_map<uint64_t,word_stats> *wordFreq = d->wordFreq;
...
uint64_t mt_process_file(Args& arg, unordered_map<uint64_t,word_stats>& wf)
{
  // get input file size 
