...
static void save_update_word(
    Args &arg, uint64_t currenWordLength, uint64_t hash,
    concurrent_unordered_map<uint64_t, concurrent_set<uint32_t>> &freq,
...
static void save_update_word(
    Args &arg, uint64_t currentSequence, uint64_t hash,
    concurrent_unordered_map<uint64_t, concurrent_set<uint32_t>> &freq,
    FILE *tmp_parse_file, FILE *last, FILE *sa, uint64_t &pos) {
...
uint64_t process_file(
  ...
  save_update_word(arg, seqNumber, currentHash, wordFreq, g, last_file, sa_file,
                   pos);
  f.close();
  return seqNumber;
}
