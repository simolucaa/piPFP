...
typedef struct {
  concurrent_unordered_map<uint64_t,concurrent_set<uint32_t>> *wordFreq; // shared dictionary
...
void *mt_parse(void *dx)
{
  ...
  Args *arg = d->arg;
  concurrent_unordered_map<uint64_t,concurrent_set<uint32_t>> *wordFreq = d->wordFreq;
  ...
  while( (c = f.get()) != EOF ) {
    ...
    // word.append(1,c);
    if (c == '\n') {
      save_update_word(*arg,d->startingSeqNumber,currentHash,*wordFreq,d->parse,d->last,d->sa,pos);
      currentWordLength = arg->w;
      currentHash = kr_hash(krw.get_window());
      d->words++;
      d->parsed++;
      if(d->start+d->parsed>=d->end) {
        cout << "Thread " << d->start << " with startingSeqNumber = " << d->startingSeqNumber << " has finished" << endl;
        f.close(); 
        return NULL;
      }
      d->startingSeqNumber++;
...
uint32_t mt_process_file(Args& arg, concurrent_unordered_map<uint64_t,concurrent_set<uint32_t>>& wf)
{
  // get input file size 
  ...
  for(int i=0;i<arg.th;i++) {
    ...
    td[i].end = i == arg.th ? size : line_pos[ranges[i+1]];
    td[i].startingSeqNumber = ranges[i];

    cout << "Thread " << i << " starts at (" << ranges[i] << ") " << td[i].start << " and ends at (" << ranges[i+1] << ") " << td[i].end << endl;
  ...
  for(int i=0;i<arg.th;i++) {
    ...
    cout << "Thread " << i << " has td[i].startingSeqNumber = " << td[i].startingSeqNumber << endl;
  }
